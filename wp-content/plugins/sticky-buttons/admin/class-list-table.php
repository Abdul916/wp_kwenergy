...
class Wow_List_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$text          = 'sticky-buttons';
		$title         = ! empty( $item['title'] ) ? $item['title'] : esc_attr__( 'Untitle', 'sticky-buttons' );
		...
		$actions       = array(
			'edit'      => '<a href="' . esc_url( $edit_url ) . '">' . esc_attr__( 'Edit', 'sticky-buttons' ) . '</a>',
			'duplicate' => '<a href="' . esc_url( $duplicate_url ) . '" style="color:green;">' . esc_attr__( 'Duplicate', 'sticky-buttons' )
			               . '</a>',
			'delete'    => '<a href="' . esc_url( wp_nonce_url($delete_url, $slug . '_nonce' ) ) . '" style="color:red;">' . esc_attr__( 'Delete', 'sticky-buttons' ) . '</a>',
			'export'    => '<a href="' . esc_url( $export_url ) . '" class="has-text-warning">' . esc_attr__( 'Export', 'sticky-buttons' ) . '</a>',
		);
	...
	private function table_data() {
		...
		if ( $result ) {
			foreach ( $result as $key => $value ) {
				...
				$title        = ! empty( $value->title ) ? $value->title :  esc_attr__( 'Untitle', 'sticky-buttons' ) ;
				$check_status = isset( $param['menu_status'] ) ? $param['menu_status'] : 1;
				$status_on      = '<span style="color:darkgreen;">' . esc_attr__( 'Active', 'sticky-buttons' ) . '</span>';
				$status_off     = '<span style="color:darkred;">' . esc_attr__( 'Inactive', 'sticky-buttons' ) . '</span>';;
	...
	public function get_bulk_actions() {
		$actions = array(
			'delete-items' => esc_attr__('Delate',  'sticky-buttons'),
		);
